...
/\.(css|css\.map|png|jpg|jpeg|svg|gif|webp)$/i.test(file)) // Include images & CSS
        .map((file) => {
...
console.log("Files Found:", fs.existsSync(imageDir) ? fs.readdirSync(imageDir).filter(file =>
file !== '.DS_Store') : "Directory Not Found");
